...
export default function TrackDetailScreen({ route, navigation }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      fetchTrack(id),
      supabase.from('item_ratings').select('*').eq('target_type', 'track').eq('target_id', id).single(),
  ...
  return (
    <ScrollView style={{ backgroundColor: '#0a0a0a' }} contentContainerStyle={{ padding: 20, paddingBottom: 60 }}>
      ...
      <TouchableOpacity
        ...
        style={{ backgroundColor: '#6C47FF', borderRadius: 10, paddingVertical: 13, alignItems: 'center', marginTop: 24 }}
      >
